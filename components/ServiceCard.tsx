...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, isFocused, onFocus, onClick }) => {
  return (
    <div 
      className={`relative group cursor-pointer flex-shrink-0 transition-all duration-500 ease-out
        ${isFocused ? 'w-[300px] z-20' : 'w-[220px] z-10 opacity-60'}
      `}
      onMouseEnter={onFocus}
      onClick={onClick}
    >
      <div className={`relative w-full h-[400px] rounded-2xl overflow-hidden transition-all duration-500 ease-out
        ${isFocused 
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 space-y-3">
          ...
          <div className="space-y-1">
            ...
            <p className={`text-slate-300 text-xs transition-all duration-500 leading-relaxed overflow-hidden
              ${isFocused ? 'opacity-100 max-h-20 translate-y-0' : 'opacity-0 max-h-0 translate-y-4'}
            `}>
              {service.description}
            </p>
          </div>
